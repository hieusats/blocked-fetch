...
<h1>Page A</h1><p>Alpha content alpha.</p><a href="/index.html">home</a>
</body></html>
```
...
async function crawlBFS(seedRaw, o) {
  ...
  while (queue.length && fetched < o.limit) {
    ...
    if (r.status === 'http:304') { // 304 = bucket unchanged, KHÔNG phải failure (spec §6.6)
    ...
    if (o.changedOnly) {
      ...
      if (hash && stt[url] && stt[url].hash === hash) { sum.unchanged++; appendRow({ url, finalUrl: r.finalUrl, file: null, title: null, hash, status: 'unchanged', via: r.via, ms: r.ms, ts: Date.now() }); enqueueIndexChildren(); continue; }
    }
...
async function cmdExtract(argv) { // element {text, href?} — textContent (jsdom không có innerText), cap 300/500 (spec §4)
  const pairs = []; let url = '';
  for (let i = 0; i < argv.length; i++) { const a = argv[i];
    if (a === '--selector') { const [name, css] = argv[++i].split('='); pairs.push({ name, css }); } else url = a; }
...
async function cmdSearch(argv) { // bare array; --scrape → JSONL envelope + delay 1500ms (spec §5)
  ...
  for (let i = 0; i < sr.results.length; i++) {
    await scrapeOne(sr.results[i].url); // dựng envelope như cmdScrape nhưng in 1 dòng JSON
...
```
(`scrapeOne` = phần lõi cmdScrape tách hàm nhận url, trả envelope — refactor nhẹ khi làm Task 9, in từng envelope 1 dòng JSONL; đặt `anyBad = true` khi envelope status ≠ ok.)
