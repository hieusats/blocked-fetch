...
async function fetch(url, opts = {}) {
  ...
  catch (err) { return { ok: false, status: 'error:' + String(err.message || err).slice(0, 200), via: 'browser', hopped: false, finalUrl: url, contentType: '', etag: null, lastModified: null, bytes: Buffer.alloc(0), html: null, text: null, ms: Date.now() - t0 }; } // spec §4: exception sanitize thành error:msg
}
...
async function derivePayload(r, flags) {
  ...
  if (ctJson(r.contentType)) { const j = safeJson(r.bytes.toString('utf8')); if (j !== null) return { key: 'json', val: j }; } // parse-fail → rơi xuống markdown (spec §4)
...
async function cmdSearch(argv) { // bare array; --scrape → JSONL envelope + delay 1500ms (spec §5)
  ...
  for (let i = 0; i < sr.results.length; i++) {
    const env = await scrapeOne(sr.results[i].url); // scrapeOne(url) TRẢ VỀ envelope, KHÔNG in — cmdSearch tự in (hợp đồng pin)
