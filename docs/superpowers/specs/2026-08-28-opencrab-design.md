...
Ngày: 2026-08-28 · Phiên bản spec: v1.8 (adversarial vòng 8 đã sửa) · Nền: council 3-advisor (Appendix A) + vòng lặp tấn công (Appendix B)
...
2. **Sitemap discovery**: thử `/sitemap.xml` + dòng `Sitemap:` trong robots.txt (curl trước); URL từ sitemap **lọc same-host** trước khi enqueue. **Seed fetch không `ok` → ghi dòng seed rồi kết thúc** (không sitemap, không BFS — allowed host không chốt được).
