...
Ngày: 2026-08-28 · Phiên bản spec: v1.13 FINAL (vòng lặp tấn công 12 vòng đã đóng) · Nền: council 3-advisor (Appendix A) + vòng lặp tấn công (Appendix B)
...
scripts/
  fetch.js        # wrapper backcompat — stdout GIỮ ĐÚNG FORMAT CŨ (không envelope):
                  ...
                  #   --max-bytes → cắt như cũ (str.slice, code units); non-200 → body nếu có, ngược lại "(http N)" (khớp v1); blocked → stdout rỗng + stderr hint + exit 1; error:msg → stdout rỗng + stderr msg + exit 1 (exit-0-silent-error của v1 là bug, sửa documented); --wait MS → opts.waitMs;
  ...
  lib/
    fetcher.js    # thang leo dời từ fetch.js: curl-UA → browser (+ search-hop khi bị chặn);
                  # --stealth = cloak THAY rung browser (không phải rung 4)
...
- `opts = { stealth (thay rung browser bằng cloak — thiếu cloakbrowser → stderr + exit 2, cả wrapper lẫn scrape, khớp v1), waitMs (settle, mặc định 1500), waitFor (selector; timeout 30s → proceed + stderr), screenshot (path), conditional: { etag, lastModified }, forceBrowser (chỉ wrapper dùng cho --selector cũ) }` — chỉ thế; robots/delay/limit là trách nhiệm caller (crawl engine).
...
- `--raw` in payload trần thay envelope (payload `null` → stdout rỗng); `--out F` ghi đúng format đã chọn (envelope mặc định; **envelope không-ok vẫn ghi** — payload `null` — khác wrapper là không ghi). `--max-bytes` (mặc định 200000): chế độ envelope chỉ cắt **trường payload — N ký tự của chuỗi payload** (envelope vẫn là JSON hợp lệ) + stderr warning trỏ `--out`; `--raw` cắt byte như cũ; không bao giờ áp cho file (scrape `--out` lẫn file crawl).
...
2. **Sitemap discovery**: thử `/sitemap.xml` + dòng `Sitemap:` trong robots.txt (curl trước); URL từ sitemap **lọc same-host** trước khi enqueue. Gặp **sitemapindex** → theo đệ quy `<sitemap><loc>` (fetch như sitemap, không phải trang); chỉ `<url><loc>` là trang. Thứ tự enqueue (FIFO): link trang theo thứ tự tài liệu trước, URL sitemap sau. **Seed fetch không `ok` → ghi header + dòng status của seed rồi kết thúc** (không sitemap, không BFS — allowed host không chốt được; dòng lỗi non-terminal nên `--resume` fetch lại seed).
...
- **Vòng 12 — TERMINAL** (b5806a3c, 6 P2: 3 chặn + 3 [sub-bar]): sitemapindex đệ quy + chỉ `<url><loc>` là trang, thứ tự enqueue FIFO (link trước, sitemap sau), --stealth = cloak THAY rung browser + exit 2 khi thiếu (khớp v1) + sửa comment ladder §3; [sub-bar]: wrapper max-bytes = str.slice code units, scrape --out vẫn ghi envelope không-ok, precedence unchanged-trước-dup. Tất cả sửa trong v1.13.

**KẾT LUẬN VÒNG LẶP**: đóng sau 12 vòng review + 3 lần parent full-read = 15 lượt tấn công độc lập; **140 lỗ hổng đã sửa** (v1.0 → v1.13). Zero P0 toàn vòng; zero P1 trong 3 vòng cuối (R10–R12); mọi finding vòng cuối là one-liner. Residue còn lại: 3 mục [sub-bar] corner-exotic (đã pin luôn) + đuôi tiềm ẩn vô hạn của lớp "chưa pin một dòng" — đã chuyển giao cho tầng phòng thủ kế: S1–S3 verify-list (§9) + selftest fixture deterministic. Định nghĩa "100% tự tin" đạt được ở đây = *hai implementation trung thành với spec không còn tạo hành vi phân kỳ quan sát được trên mọi đường dùng thực tế mà 15 lượt tấn công tìm ra được*.
