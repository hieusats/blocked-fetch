...
const { fetch, extractLinks, close, SetupError, stateDir } = require('./fetcher');
...
async function crawlBFS(seedRaw, o) {
  ...
  const statePath = host => path.join(stateDir(), 'state', host + '.json'); // cùng gốc với pidfile/profile (fetcher) — một nguồn
  ...
  while (queue.length && fetched < o.limit) {
    ...
    if (!r.ok) {
      ...
      if (url === seed) { sum.seedFailed = true; break; } // seed không ok → dừng (spec §6.2); exit 1 — crawl rỗng không phải thành công
      continue;
    }
...
async function runExtract(url, pairs, opts, emit) { // fetch + jsdom selectors; return ok (Task 6 wrapper dùng)
  ...
  const r = await fetch(url, opts); // module-level import — bỏ re-require
