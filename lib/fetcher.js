...
async function findHop(p, host, settle) {
  for (const [name, [base, linkSel]] of Object.entries(ENGINES)) {
    await doGotoTo(p, base + encodeURIComponent('site:' + host), settle);
    const body = await p.evaluate(() => document.body ? document.body.innerText : '').catch(() => '');
    if (CHALLENGE_RE.test(body.slice(0, 400))) continue;
    const href = await p.evaluate(ls => document.querySelector(ls)?.href || '', linkSel).catch(() => '');
    if (href) { console.error(`[#] hopping via ${name}`); return href; }
  }
  return '';
}
// search 3-engine (spec §3 rung search): DDG HTML/lite + Bing — MỘT BẢNG cho cả findHop lẫn searchResults; fallback theo thứ tự khi bị challenge
const ENGINES = {
...
const CHALLENGE_RE = /challenge|captcha|squares containing/i;
async function searchResults(q, { limit = 10 } = {}) {
  const p = await ensureBrowser(false);
  for (const [name, [base, linkSel, snipSel]] of Object.entries(ENGINES)) {
...
function extractLinks(html, baseUrl) {
  const out = [];
  // \shref (không phải data-href/ng-href); 3 nhánh quote: "…" | '…' | không-quote; '#' giữ nguyên trong capture — normalizeUrl strip sau
  const re = /<a\b[^>]*?\shref\s*=\s*(?:"([^"]*)"|'([^']*)'|([^\s>]+))/gi;
  let m; const base = new URL(baseUrl);
  while ((m = re.exec(html))) { try { out.push(new URL(m[1] ?? m[2] ?? m[3], base).href); } catch {} }
  return out;
}
module.exports = { fetch, close, extractLinks, searchResults, SetupError, stateDir }; // UA/BLOCK_PAT dùng nội bộ
